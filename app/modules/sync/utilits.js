...
/**
 * Рассылка уведомлений, которые были не получены ранее
 * @param {*} socket 
 * @param {*} io 
 */
exports.messages = function(socket, io) {
    db.provider.db().query(`
        SELECT id, ba_data, c_data_type, c_from, b_temporary, dx_created
        FROM public.socket_io_messages
        WHERE b_delivered = false AND c_to = $1;`, [socket.user.id], (err, res) => {
            if(err) {
                ...
                if(count > 0) {
                    for(var i = 0; i < count; i++) {
                        var row = res.rows[i];
                        io.emitter.to(socket.user.id).emit('messages', row.id, row.ba_data, row.c_data_type, row.b_temporary, row.dx_created, row.c_from);
                    }
                    Console.debug(`${socket.user.id} : ${count}`, 'Socket.IO', socket.user.id, socket.user.c_claims, null, null, 'messages');
                }
            }
        });
}
...
function log(socket, tid) {
    return new function() {
        ...
        this.error = function(message) {
            send(socket, message, tid);
            Console.error(message, 'SOCKET_ERROR');
            return message;
        }
...
exports.socketResultLayout = function(obj, tid) {
    return socketOkResultLayout(obj, tid);
}
...
exports.writeFile = function (session) {
    ...
    return new function () {
        ...
        this.toError = function (record, message) {
            ...
            Console.error(record.__error, 'SOCKET_ERROR');
...
exports.progressFile = function (logger, totalCount) {
    return new function () {
        ...
        this.init = function (time) {
            Console.debug('Получено из БД ' + totalCount + ' файлов за ' + time + ' мс.', 'SOCKET_LOG', null, null);
        }
        ...
        this.finish = function (message) {
            ...
            Console.debug(avgMessage, 'SOCKET_LOG');
